...
public:
    ...
    /// @return True if objects are considered equal, false otherwise.
    template<typename T, typename T2>
    static typename std::enable_if<!IsExceptionWrapper<T>::value && !IsSmartPtr<T>::value && !std::is_scalar<T>::value && !::System::Details::HasIsNull<T>::value, bool>::type Equals(T obj, const T2& another)
    {
        return obj.Equals(another);
    }
