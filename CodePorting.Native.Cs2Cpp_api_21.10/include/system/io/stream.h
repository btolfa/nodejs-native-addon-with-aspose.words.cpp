...
protected:
    ...
    /// Reads the specified number of bytes from the stream and writes them to the specified buffer.
    /// @param buffer The buffer to write bytes to.
    /// @param offset A 0-based position in @p buffer to start writing at.
    /// @param count The number of bytes to read.
    /// @returns The number of bytes read.
    virtual ASPOSECPP_SHARED_API int32_t ReadImpl(const ArrayPtr<uint8_t>& buffer, int32_t offset, int32_t count);
};
