...
public:
    ...
    /// Constructs string based on character array. Treats array as null-terminated string in UTF8, calculates target string length based on array size.
    /// @param value Character array pointer.
    template<typename T>
    explicit String(T& value, typename std::enable_if<IsNonConstStringArray<T, char>::value>::type* = nullptr)
        : String(value, std::extent<T>::value - 1)
    {}
    ...
    explicit ASPOSECPP_SHARED_API operator std::string() const;
    /// Implicit conversion from Aspose System::String to std::wstring.
    /// @return std::wstring
    explicit ASPOSECPP_SHARED_API operator std::wstring() const;
    /// Implicit conversion from Aspose System::String to std::u16string.
    /// @return std::u16string
    explicit ASPOSECPP_SHARED_API operator std::u16string() const;
    /// Implicit conversion from Aspose System::String to std::u32string.
    /// @return std::u32string
    explicit ASPOSECPP_SHARED_API operator std::u32string() const;
...
template <typename T>
struct MakeConstRef : std::conditional<IsSmartPtr<T>::value || std::is_same<System::String, T>::value, const T&, T> {};
